...
# aster CLI and Server Docker Image
...
RUN cargo build --release --package aster-cli
...
COPY --from=builder /build/target/release/aster /usr/local/bin/aster

# Create non-root user
RUN useradd -m -u 1000 -s /bin/bash aster && \
    mkdir -p /home/aster/.config/aster && \
    chown -R aster:aster /home/aster
