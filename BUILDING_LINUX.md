# Building aster Desktop on Linux

This guide covers building the aster Desktop application from source on various Linux distributions.
...
Output: `out/make/zip/linux/x64/aster-linux-x64-{version}.zip`
...
Output: `out/make/deb/x64/aster_{version}_amd64.deb`
...
```bash
./out/aster-linux-x64/aster
```

#### Install DEB Package (if built)
```bash
sudo dpkg -i out/make/deb/x64/aster_*.deb
```
...
```bash
#!/bin/bash
cd /path/to/aster/ui/desktop/out/aster-linux-x64
./aster 2>&1 | grep -v "GLib-GObject" | grep -v "browser_main_loop"
```

#### Server Binary Not Found
If you see "Could not find asterd binary", ensure you've:
1. Built the Rust backend: `cargo build --release -p aster-server`
2. Copied it to the right location: `cp ../../target/release/asterd src/bin/`
...
1. **Backend changes**: Rebuild with `cargo build --release -p aster-server` and copy the binary
...
### Desktop Entry
Create `~/.local/share/applications/aster.desktop`:
```ini
[Desktop Entry]
Name=aster AI Agent
Comment=Local AI agent for development tasks
Exec=/path/to/aster/ui/desktop/out/aster-linux-x64/aster %U
Icon=/path/to/aster/ui/desktop/out/aster-linux-x64/resources/app.asar.unpacked/src/images/icon.png
Terminal=false
Type=Application
Categories=Development;Utility;
StartupNotify=true
MimeType=x-scheme-handler/aster
```

### System-wide Installation
To install system-wide:
```bash
sudo cp -r out/aster-linux-x64 /opt/aster
sudo ln -s /opt/aster/aster /usr/local/bin/aster-gui
```
