    ...
    cat > "$OUTPUT_DIR/scan_status.json" << EOF
{
  ...
  "aster_exit_code": ${SCAN_EXIT_CODE:-0},
  "debug_info": {
    "line": ${line_no},
    "exit_code": ${exit_code},
    ...
    "environment": {
      ...
      "aster_exists": $([ -f "$ASTER_BIN" ] && echo "true" || echo "false"),
...
🔍 Aster Recipe Security Scanner - ERROR REPORT
==============================================
...
- Aster binary exists: $([ -f "$ASTER_BIN" ] && echo "✅ YES" || echo "❌ NO")
...
4. Aster configuration
EOF

    # Ensure aster_output.log exists
    touch "$OUTPUT_DIR/aster_output.log" 2>/dev/null || true
...
if python3 /usr/local/bin/decode-training-data.py; then
    ...
    TRAINING_INSTRUCTIONS="/tmp/aster_training_instructions.md"
...
if [ ! -f "$ASTER_BIN" ]; then
       ...
       https://github.com/astercloud/aster-rust/releases/download/stable/download_cli.sh | bash; then
        for path in "$HOME/.local/bin/aster" "/usr/local/bin/aster" "$(which aster 2>/dev/null || true)"; do
            if [ -n "$path" ] && [ -f "$path" ] && [ -x "$path" ]; then
                cp "$path" "$ASTER_BIN"
                chmod +x "$ASTER_BIN"
                echo "✅ Aster CLI installed from $path"
                break
            fi
        done
    fi
    ...
    if [ ! -f "$ASTER_BIN" ]; then
        ...
        if curl -fsSL --connect-timeout 30 --max-time 300 \
           ...
           -o "$temp_dir/aster.tar.bz2"; then
            ...
            if [ -n "$aster_binary" ]; then
                cp "$aster_binary" "$ASTER_BIN"
                chmod +x "$ASTER_BIN"
                echo "✅ Aster CLI installed via direct download"
            fi
        fi
        rm -rf "$temp_dir"
    fi
    ...
    if [ ! -f "$ASTER_BIN" ]; then
        echo "❌ Failed to install Aster CLI"
        exit 1
    fi
fi

# Verify Aster installation
echo "🔧 Verifying Aster installation..."
if ! "$ASTER_BIN" --version >/dev/null 2>&1; then
    echo "❌ Aster CLI not working"
    "$ASTER_BIN" --version || true
    exit 1
fi

echo "✅ Aster CLI ready: $($ASTER_BIN --version)"

# Set up Aster environment
echo "🔧 Configuring Aster environment..."

USER_ID="$(id -u)"
ASTER_TMP="/tmp/aster_${USER_ID}"
mkdir -p "$ASTER_TMP"/{logs,state,cache,config} 2>/dev/null || true
chmod -R 755 "$ASTER_TMP" 2>/dev/null || true

export XDG_STATE_HOME="$ASTER_TMP/state"
export XDG_CACHE_HOME="$ASTER_TMP/cache"
export ASTER_TELEMETRY_ENABLED=false
export ASTER_PROJECT_TRACKER_ENABLED=false
export RUST_LOG=error

if [ -f "$HOME/.config/aster/config.yaml" ]; then
    cp "$HOME/.config/aster/config.yaml" "$ASTER_TMP/config/config.yaml" 2>/dev/null || true
    export ASTER_CONFIG_DIR="$ASTER_TMP/config"
fi

echo "✅ Aster environment configured"

# Quick health check (decoupled from analysis)
echo "🔍 Running Aster health check..."
if timeout 30 "$ASTER_BIN" run --no-session -t "Hello, are you working?" >> "$OUTPUT_DIR/aster_output.log" 2>&1; then
    echo "✅ Aster health check passed"
else
    echo "⚠️ Aster health check failed - continuing anyway"
fi
...
python3 - "$RECIPE_FILE" > "$PY_UNICODE_REPORT" 2>>"$OUTPUT_DIR/aster_output.log" <<'PY' || true
import sys, json
path = sys.argv[1]
...
if [ -s "$PY_UNICODE_REPORT" ] && jq -e '.findings | length > 0' "$PY_UNICODE_REPORT" >/dev/null 2>&1; then
  ...
  jq -n \
    ...
    --argjson aster_exit_code 0 \
    --arg analysis_method "${ANALYSIS_METHOD}" \
    '{status: $status, reason: $reason, risk_score: $risk_score, risk_level: $risk_level, message: $message, scan_successful: $scan_successful, analysis_method: $analysis_method, aster_exit_code: $aster_exit_code}' \
  ...
  jq -n \
    ...
    '{path_taken:$path_taken, markers_found:$markers_found, retry_attempted:$retry_attempted, heuristic_used:$heuristic_used, unicode_found:$unicode_found, benign_hint:$benign_hint, aster_exit_code:$aster_exit_code, timestamp:$timestamp, unicode_scan:$unicode_findings}' \
    > "$OUTPUT_DIR/analysis_meta.json"

  # Reports
  TIMESTAMP=$(date -u -Iseconds)
  cat > "$OUTPUT_DIR/security-report.md" << EOF
# Aster Recipe Security Analysis
...
$(jq -r '.[]? | "- " + (.category // "unicode") + ": " + (.snippet // "")' <<< "$EVIDENCE" 2>/dev/null || echo "See aster_result.json")
...
EOF
  ...
  cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Aster Recipe Security Analysis Summary (Unicode Stealth)
=========================================================
...
if [ "${BENIGN_HINT}" = true ]; then
  ...
  jq -n \
    ...
    --argjson aster_exit_code 0 \
    --arg timestamp "$(date -u -Iseconds)" \
    '{path_taken:$path_taken, markers_found:$markers_found, retry_attempted:$retry_attempted, heuristic_used:$heuristic_used, unicode_found:$unicode_found, benign_hint:$benign_hint, aster_exit_code:$aster_exit_code, timestamp:$timestamp}' \
    > "$OUTPUT_DIR/analysis_meta.json"
...
EOF
  ...
  cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Aster Recipe Security Analysis Summary (Deterministic Benign)
==============================================================
📅 Analysis Date: $(date -u)
📋 Recipe: $(basename "$RECIPE_FILE")
🤖 Analysis Method: Deterministic benign fallback
...
if timeout 60 "$ASTER_BIN" run \
    --recipe "$BASE_RECIPE" \
    --no-session \
    --render-recipe \
    --params recipe_path="$RECIPE_FILE" \
    --params strict_mode="false" \
    > "$OUTPUT_DIR/rendered_base_recipe.yaml" 2>> "$OUTPUT_DIR/aster_output.log"; then
  echo "✅ Rendered base recipe saved to $OUTPUT_DIR/rendered_base_recipe.yaml"
else
  echo "⚠️ Failed to render base recipe (non-fatal)" >> "$OUTPUT_DIR/aster_output.log"
fi
...
timeout 600 "$ASTER_BIN" run \
    --recipe "$BASE_RECIPE" \
    --no-session \
    --quiet \
    --params recipe_path="$RECIPE_FILE" \
    >> "$OUTPUT_DIR/aster_output.log" 2>&1 || SCAN_EXIT_CODE=$?

echo "📊 Security analysis completed with exit code: $SCAN_EXIT_CODE"

# Parsing helpers
extract_marked_json() {
  if grep -q 'BEGIN_ASTER_JSON' "$OUTPUT_DIR/aster_output.log" && grep -q 'END_ASTER_JSON' "$OUTPUT_DIR/aster_output.log"; then
    MARKERS_FOUND=true
    tac "$OUTPUT_DIR/aster_output.log" | awk '
        /END_ASTER_JSON/ && !found { found=1; next }
        found && /BEGIN_ASTER_JSON/ { exit }
        found { print }
    ' | tac > "$OUTPUT_DIR/aster_result.marked.txt" 2>/dev/null || true
    # strip code fences and blank lines
    sed -e 's/^```[a-zA-Z]*$//g' -e 's/^```$//g' "$OUTPUT_DIR/aster_result.marked.txt" | sed '/^\s*$/d' > "$OUTPUT_DIR/aster_result.json" || true
  fi
}

heuristic_json() {
  PY_OUT="$OUTPUT_DIR/aster_result.heuristic.json"
  python3 - "$OUTPUT_DIR/aster_output.log" > "$PY_OUT" 2>>"$OUTPUT_DIR/aster_output.log" <<'PY' || true
import sys, json
path = sys.argv[1]
...
PY
  if [ -s "$PY_OUT" ] && jq . "$PY_OUT" >/dev/null 2>&1; then
    mv -f "$PY_OUT" "$OUTPUT_DIR/aster_result.json" || true
    HEURISTIC_USED=true
  fi
}
...
# Try markers
extract_marked_json
if [ -f "$OUTPUT_DIR/aster_result.json" ] && jq . "$OUTPUT_DIR/aster_result.json" >/dev/null 2>&1; then
  JSON_VALID=true
else
  # Heuristic attempt 1
  heuristic_json
...
if [ "$JSON_VALID" = false ]; then
  ...
  echo "🔁 Retrying once with strict JSON-only instruction..." | tee -a "$OUTPUT_DIR/aster_output.log"
  timeout 120 "$ASTER_BIN" run \
      --recipe "$BASE_RECIPE" \
      --no-session \
      --params recipe_path="$RECIPE_FILE" \
      --params strict_mode="true" \
      >> "$OUTPUT_DIR/aster_output.log" 2>&1 || true
...
if [ "$JSON_VALID" = true ]; then
  echo "✅ Found valid JSON result from Aster"
  SCORE=$(jq -r '.score // 0' "$OUTPUT_DIR/aster_result.json")
  RECOMMENDATION=$(jq -r '.recommendation // "UNKNOWN"' "$OUTPUT_DIR/aster_result.json")
  SUMMARY=$(jq -r '.summary // "No summary provided"' "$OUTPUT_DIR/aster_result.json")
  if ! [[ "${SCORE}" =~ ^[0-9]+$ ]]; then
    echo "⚠️ SCORE not numeric ('$SCORE'); entering fallback scoring" | tee -a "$OUTPUT_DIR/aster_output.log"
    JSON_VALID=false
  else
    SCAN_SUCCESSFUL=true
    # Only set to aster_ai if we did not set a more specific method above
    if [ "$ANALYSIS_METHOD" = "aster_ai" ]; then
      ANALYSIS_METHOD="aster_ai"
    fi
  fi
fi
...
if [ "$JSON_VALID" != true ]; then
  echo "🧮 Applying enhanced static fallback scoring" >> "$OUTPUT_DIR/aster_output.log"
  RAW=$(tr '\n' ' ' < "$RECIPE_FILE" | tr -s ' ')
  SCORE=15
  ...
  # Check for external script downloads - analyze the actual content
    ...
    echo "🔍 Static analysis: found external URLs, analyzing content..." >> "$OUTPUT_DIR/aster_output.log"
    echo "🔍 Detected URLs: $EXTERNAL_URLS" >> "$OUTPUT_DIR/aster_output.log"
    ...
    for URL in $EXTERNAL_URLS; do
      ...
      echo "🔍 Downloading: $URL" >> "$OUTPUT_DIR/aster_output.log"
      
      # Enhanced curl with better error handling and user agent
      if curl -sSfL --max-time 30 --connect-timeout 10 --user-agent "Mozilla/5.0 (Security Scanner)" "$URL" -o "$SCRIPT_FILE" 2>>"$OUTPUT_DIR/aster_output.log"; then
        echo "✅ Downloaded external file for analysis: $URL ($(wc -c < "$SCRIPT_FILE") bytes)" >> "$OUTPUT_DIR/aster_output.log"
        SCRIPT_ANALYZED=true
        
        # Show first few lines for debugging
        echo "📄 First 5 lines of downloaded content:" >> "$OUTPUT_DIR/aster_output.log"
        head -5 "$SCRIPT_FILE" >> "$OUTPUT_DIR/aster_output.log" 2>/dev/null || echo "Could not read file" >> "$OUTPUT_DIR/aster_output.log"
        ...
        echo "🔍 Detailed analysis of $URL:" >> "$OUTPUT_DIR/aster_output.log"
        echo "  - SSH patterns: $SSH_MATCHES" >> "$OUTPUT_DIR/aster_output.log"
        echo "  - POST/exfil patterns: $POST_MATCHES" >> "$OUTPUT_DIR/aster_output.log"
        echo "  - Credential patterns: $CRED_MATCHES" >> "$OUTPUT_DIR/aster_output.log"
        echo "  - Network patterns: $NET_MATCHES" >> "$OUTPUT_DIR/aster_output.log"
        echo "  - Cleanup patterns: $CLEANUP_MATCHES" >> "$OUTPUT_DIR/aster_output.log"
        ...
        if [ "$SSH_MATCHES" -gt 0 ]; then
          if [ 95 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: SSH key access in external script: $URL" >> "$OUTPUT_DIR/aster_output.log"
          fi
        fi
        ...
        if [ "$POST_MATCHES" -gt 0 ]; then
          if [ 90 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: Data exfiltration in external script: $URL" >> "$OUTPUT_DIR/aster_output.log"
          fi
        fi
        ...
        if [ "$CRED_MATCHES" -gt 0 ] && [ "$NET_MATCHES" -gt 0 ]; then
          if [ 85 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: Credential theft in external script: $URL" >> "$OUTPUT_DIR/aster_output.log"
          fi
        fi
        ...
        if [ "$CLEANUP_MATCHES" -gt 0 ]; then
          if [ 75 -gt $HIGHEST_SCORE ]; then
            ...
            echo "⚠️ HIGH RISK DETECTED: Stealth cleanup operations in external script: $URL" >> "$OUTPUT_DIR/aster_output.log"
          fi
        fi
        ...
        if [ "$PKG_MATCHES" -gt 0 ] && [ 40 -gt $HIGHEST_SCORE ]; then
          ...
          echo "📦 MEDIUM RISK DETECTED: Package installation in external script: $URL" >> "$OUTPUT_DIR/aster_output.log"
        fi
        
      else
        CURL_EXIT_CODE=$?
        echo "⚠️ Failed to download: $URL (curl exit code: $CURL_EXIT_CODE)" >> "$OUTPUT_DIR/aster_output.log"
        echo "🔍 Trying alternative download method..." >> "$OUTPUT_DIR/aster_output.log"
        
        # Try with wget as fallback
        if command -v wget >/dev/null 2>&1 && wget --timeout=30 --tries=2 -q "$URL" -O "$SCRIPT_FILE" 2>>"$OUTPUT_DIR/aster_output.log"; then
          echo "✅ Downloaded via wget: $URL" >> "$OUTPUT_DIR/aster_output.log"
          SCRIPT_ANALYZED=true
          # Repeat analysis logic here if needed
        else
          echo "❌ All download methods failed for: $URL" >> "$OUTPUT_DIR/aster_output.log"
...
TAIL_OUT="$OUTPUT_DIR/aster_output_tail.txt"
tail -n 300 "$OUTPUT_DIR/aster_output.log" > "$TAIL_OUT" 2>/dev/null || true
...
jq -n \
  ...
  --argjson aster_exit_code ${SCAN_EXIT_CODE:-0} \
  --arg analysis_method "${ANALYSIS_METHOD}" \
  '{status: $status, reason: $reason, risk_score: $risk_score, risk_level: $risk_level, message: $message, scan_successful: $scan_successful, analysis_method: $analysis_method, aster_exit_code: $aster_exit_code}' \
  > "$OUTPUT_DIR/scan_status.json"
...
- Aster Exit Code: $SCAN_EXIT_CODE
- Timestamp: $TIMESTAMP

## Evidence

$(jq -r '.evidence[]? | "- " + (.category // "evidence") + ": " + (.snippet // "")' "$OUTPUT_DIR/aster_result.json" 2>/dev/null || echo "See aster_result.json for detailed evidence")

## Artifacts

- scan_status.json - Machine-readable scan status
- aster_result.json - Complete analysis results
- aster_output.log - Full analysis execution log
...
cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Aster Recipe Security Analysis Summary
========================================
...
📋 Available Reports:
  ...
  • aster_result.json - Analysis results
  • aster_output.log - Log
