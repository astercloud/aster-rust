...
release-binary:
    ...
    cargo run -p aster-server --bin generate_schema
...
release-windows:
    ...
    if [ "$(uname)" = "Darwin" ] || [ "$(uname)" = "Linux" ]; then
        ...
        docker volume create aster-windows-cache || true
        docker run --rm \
            -v "$(pwd)":/usr/src/myapp \
            -v aster-windows-cache:/usr/local/cargo/registry \
            -w /usr/src/myapp \
            rust:latest \
            sh -c "{{win_docker_build_sh}}"
    else
        echo "Building Windows executable using Docker through PowerShell..."
        powershell.exe -Command "docker volume create aster-windows-cache; \`
            docker run --rm \`
                -v ${PWD}:/usr/src/myapp \`
                -v aster-windows-cache:/usr/local/cargo/registry \`
                -w /usr/src/myapp \`
...
copy-binary BUILD_MODE="release":
    @if [ -f ./target/{{BUILD_MODE}}/asterd ]; then \
        echo "Copying asterd binary from target/{{BUILD_MODE}}..."; \
        cp -p ./target/{{BUILD_MODE}}/asterd ./ui/desktop/src/bin/; \
    else \
        echo "Binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi
    @if [ -f ./target/{{BUILD_MODE}}/aster ]; then \
        echo "Copying aster CLI binary from target/{{BUILD_MODE}}..."; \
        cp -p ./target/{{BUILD_MODE}}/aster ./ui/desktop/src/bin/; \
    else \
        echo "aster CLI binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi

# Copy binary command for Intel build
copy-binary-intel:
    @if [ -f ./target/x86_64-apple-darwin/release/asterd ]; then \
        echo "Copying Intel asterd binary to ui/desktop/src/bin with permissions preserved..."; \
        cp -p ./target/x86_64-apple-darwin/release/asterd ./ui/desktop/src/bin/; \
    else \
        echo "Intel release binary not found."; \
        exit 1; \
    fi
    @if [ -f ./target/x86_64-apple-darwin/release/aster ]; then \
        echo "Copying Intel aster CLI binary to ui/desktop/src/bin..."; \
        cp -p ./target/x86_64-apple-darwin/release/aster ./ui/desktop/src/bin/; \
    else \
        echo "Intel aster CLI binary not found."; \
        exit 1; \
    fi

# Copy Windows binary command
copy-binary-windows:
    @powershell.exe -Command "if (Test-Path ./target/x86_64-pc-windows-gnu/release/asterd.exe) { \
        Write-Host 'Copying Windows binary and DLLs to ui/desktop/src/bin...'; \
        Copy-Item -Path './target/x86_64-pc-windows-gnu/release/asterd.exe' -Destination './ui/desktop/src/bin/' -Force; \
...
run-ui-playwright:
    ...
    RUN_DIR="$HOME/aster-runs/$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$RUN_DIR"
    echo "Using isolated directory: $RUN_DIR"
    cd ui/desktop && ENABLE_PLAYWRIGHT=true ASTER_PATH_ROOT="$RUN_DIR" npm run start-gui
...
debug-ui *alpha:
    @echo "🚀 Starting aster frontend in external backend mode{{ if alpha == "alpha" { " with alpha features enabled" } else { "" } }}"
    cd ui/desktop && \
    export ASTER_EXTERNAL_BACKEND=true && \
    export ASTER_EXTERNAL_PORT=3000 && \
...
debug-ui-main-process:
	@echo "🔍 Starting aster UI with main process debugging enabled"
	@just release-binary
	cd ui/desktop && \
	npm install && \
...
run-server:
    @echo "Running server..."
    cargo run -p aster-server --bin asterd agent
...
make-ui-windows:
    ...
    #!/usr/bin/env sh
    set -e
    if [ -f "./target/x86_64-pc-windows-gnu/release/asterd.exe" ]; then \
        echo "Cleaning destination directory..." && \
        rm -rf ./ui/desktop/src/bin && \
        mkdir -p ./ui/desktop/src/bin && \
        echo "Copying Windows binary and DLLs..." && \
        cp -f ./target/x86_64-pc-windows-gnu/release/asterd.exe ./ui/desktop/src/bin/ && \
...
prepare-release version:
    ...
    @git add \
        ...
        crates/aster/src/providers/canonical/data/canonical_models.json \
        crates/aster/src/providers/canonical/data/canonical_mapping_report.json
...
win-bld profile allparam:
  cargo run {{profile}} -p aster-server --bin  generate_schema
