    ...
    <SharedSessionView
      ...
      onRetry={async () => {
        if (shareToken && baseUrl) {
          ...
          try {
            await openSharedSessionFromDeepLink(`aster://sessions/${shareToken}`, setView, baseUrl);
          } catch (error) {
  ...
  useEffect(() => {
    const handleOpenSharedSession = async (_event: IpcRendererEvent, ...args: unknown[]) => {
      ...
      try {
        ...
        const shareToken = link.replace('aster://sessions/', '');
        const options = {
          sessionDetails: null,
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if ((isMac ? event.metaKey : event.ctrlKey) && event.key === 'n') {
        ...
        try {
          const workingDir = window.appConfig?.get('ASTER_WORKING_DIR');
