...
import { checkServerStatus, startAsterd } from './asterd';
...
const asterTempDir = path.join(app.getPath('temp'), 'aster-pasted-images');
...
async function ensureTempDirExists(): Promise<string> {
  try {
    ...
    try {
      const stats = await fs.stat(asterTempDir);

      // If it exists but is not a directory, remove it and recreate
      if (!stats.isDirectory()) {
        await fs.unlink(asterTempDir);
        await fs.mkdir(asterTempDir, { recursive: true });
      }

      // Startup cleanup: remove old files and any symlinks
      const files = await fs.readdir(asterTempDir);
      const now = Date.now();
      const MAX_AGE = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

      for (const file of files) {
        const filePath = path.join(asterTempDir, file);
        try {
    ...
    await fs.chmod(asterTempDir, 0o755);

    console.log('[Main] Temporary directory for pasted images ensured:', asterTempDir);
  } catch (error) {
    console.error('[Main] Failed to create temp directory:', asterTempDir, error);
...
if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
  ...
  console.log('[Main] Development mode: Forcing protocol registration for aster://');
  app.setAsDefaultProtocolClient('aster');

  if (process.platform === 'darwin') {
    try {
      // Reset the default handler to ensure dev version takes precedence
      spawn('open', ['-a', process.execPath, '--args', '--reset-protocol-handler', 'aster'], {
        detached: true,
        stdio: 'ignore',
      });
    } catch {
...
if (process.platform !== 'darwin') {
  ...
  if (!gotTheLock) {
    ...
    app.on('second-instance', (_event, commandLine) => {
      const protocolUrl = commandLine.find((arg) => arg.startsWith('aster://'));
      if (protocolUrl) {
  ...
  const protocolUrl = process.argv.find((arg) => arg.startsWith('aster://'));
  if (protocolUrl) {
    app.whenReady().then(() => {
      handleProtocolUrl(protocolUrl);
    });
...
const getBundledConfig = (): BundledConfig => {
  //{env-macro-start}//
  //needed when aster is bundled for a specific provider
  //{env-macro-end}//
  return {
    defaultProvider: process.env.ASTER_DEFAULT_PROVIDER,
    defaultModel: process.env.ASTER_DEFAULT_MODEL,
    predefinedModels: process.env.ASTER_PREDEFINED_MODELS,
    baseUrlShare: process.env.ASTER_BASE_URL_SHARE,
    version: process.env.ASTER_VERSION,
  };
};
...
const getServerSecret = (settings: ReturnType<typeof loadSettings>): string => {
  if (settings.externalAsterd?.enabled && settings.externalAsterd.secret) {
    return settings.externalAsterd.secret;
  }
...
let appConfig = {
  ASTER_DEFAULT_PROVIDER: defaultProvider,
  ASTER_DEFAULT_MODEL: defaultModel,
  ASTER_PREDEFINED_MODELS: predefinedModels,
  ASTER_API_HOST: 'http://127.0.0.1',
  ASTER_WORKING_DIR: '',
  // If ASTER_ALLOWLIST_WARNING env var is not set, defaults to false (strict blocking mode)
  ASTER_ALLOWLIST_WARNING: process.env.ASTER_ALLOWLIST_WARNING === 'true',
};

const windowMap = new Map<number, BrowserWindow>();
const asterdClients = new Map<number, Client>();
...
const createChat = async (
  ...
  const asterdResult = await startAsterd({
    app,
    serverSecret,
    dir: dir || os.homedir(),
    env: { ASTER_PATH_ROOT: process.env.ASTER_PATH_ROOT },
    externalAsterd: settings.externalAsterd,
  });

  const { baseUrl, workingDir, process: asterdProcess, errorLog } = asterdResult;
  ...
  const mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      additionalArguments: [
        JSON.stringify({
          ...appConfig,
          ASTER_API_HOST: baseUrl,
          ASTER_WORKING_DIR: workingDir,
          REQUEST_DIR: dir,
          ASTER_BASE_URL_SHARE: baseUrlShare,
  ...
  const asterdClient = createClient(
    createConfig({
      baseUrl,
      headers: {
  ...
  asterdClients.set(mainWindow.id, asterdClient);

  const serverReady = await checkServerStatus(asterdClient, errorLog);
  if (!serverReady) {
    const isUsingExternalBackend = settings.externalAsterd?.enabled;

    if (isUsingExternalBackend) {
      const response = dialog.showMessageBoxSync({
        type: 'error',
        title: 'External Backend Unreachable',
        message: `Could not connect to external backend at ${settings.externalAsterd?.url}`,
        detail: 'The external asterd server may not be running.',
      ...
      if (response === 0) {
        const updatedSettings = {
          ...
          externalAsterd: {
            enabled: false,
            url: settings.externalAsterd?.url || '',
            secret: settings.externalAsterd?.secret || '',
          },
        };
  ...
  // Aster's react app uses HashRouter, so the path + search params follow a #/
  ...
  mainWindow.on('closed', () => {
    ...
    if (asterdProcess && typeof asterdProcess === 'object' && 'kill' in asterdProcess) {
      asterdProcess.kill();
    }
  });
  return mainWindow;
  ...
  if (currentWindow) {
    try {
      const currentWorkingDir = await currentWindow.webContents.executeJavaScript(
        `window.appConfig ? window.appConfig.get('ASTER_WORKING_DIR') : null`
      );
...
ipcMain.handle('get-asterd-host-port', async (event) => {
  const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
  if (!windowId) {
    return null;
  }
  const client = asterdClients.get(windowId);
  if (!client) {
    return null;
  }
...
ipcMain.handle('get-temp-image', async (_event, filePath: string) => {
  ...
  const resolvedPath = path.resolve(filePath);
  const resolvedTempDir = path.resolve(asterTempDir);
  ...
  try {
    ...
    try {
      realTempDir = await fs.realpath(asterTempDir);
...
ipcMain.on('delete-temp-file', async (_event, filePath: string) => {
  ...
  try {
    ...
    try {
      const realTempDir = await fs.realpath(asterTempDir);
      const realPath = await fs.realpath(filePath);
...
async function appMain() {
  ...
  const buildConnectSrc = (): string => {
    ...
    const settings = loadSettings();
    if (settings.externalAsterd?.enabled && settings.externalAsterd.url) {
      try {
        const externalUrl = new URL(settings.externalAsterd.url);
        sources.push(externalUrl.origin);
      } catch {
        console.warn('Invalid external asterd URL in settings, skipping CSP entry');
      }
    }
  ...
  // App menu
  const appMenu = menu?.items.find((item) => item.label === 'Aster');
  if (appMenu?.submenu) {
  ...
  if (fileMenu?.submenu) {
    ...
    // Open aster to specific dir and set that as its working space
    fileMenu.submenu.insert(
      2,
      new MenuItem({
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About Aster menu item with a submenu
      const aboutAsterMenuItem = new MenuItem({
        label: 'About Aster',
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About Aster submenu
      if (aboutAsterMenuItem.submenu) {
        aboutAsterMenuItem.submenu.append(
          new MenuItem({
      ...
      helpMenu.submenu.append(aboutAsterMenuItem);
    }
  }
  ...
  ipcMain.handle('fetch-metadata', async (_event, url) => {
    try {
      ...
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; Aster/1.0)',
        },
      });
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('Aster Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});

async function getAllowList(): Promise<string[]> {
  if (!process.env.ASTER_ALLOWLIST) {
    return [];
  }

  const response = await fetch(process.env.ASTER_ALLOWLIST);
...
app.on('will-quit', async () => {
  ...
  try {
    await fs.access(asterTempDir); // Check if directory exists to avoid error on fs.rm if it doesn't
    ...
    try {
      ...
      if (!hasSymlinks) {
        await fs.rm(asterTempDir, { recursive: true, force: true });
