...
export function extractImagePaths(text: string): string[] {
  ...
  // Pattern: /path/to/aster-pasted-images/pasted-img-TIMESTAMP-RANDOM.ext
  // This regex looks for:
  // - Word boundary or start of string
  // - A path containing "aster-pasted-images"
  // - Followed by a filename starting with "pasted-"
  // - Ending with common image extensions
  // - Word boundary or end of string
  const regex =
    /(?:^|\s)((?:[^\s]*\/)?aster-pasted-images\/pasted-[^\s]+\.(png|jpg|jpeg|gif|webp))(?=\s|$)/gi;
