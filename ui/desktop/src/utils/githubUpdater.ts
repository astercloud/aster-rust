...
export class GitHubUpdater {
    ...
    try {
      ...
      const response = await fetch(this.apiUrl, {
        headers: {
          ...
          'User-Agent': `Aster-Desktop/${app.getVersion()}`,
        },
      ...
      if (platform === 'darwin') {
        ...
        if (arch === 'arm64') {
          ...
          assetName = 'Aster_intel_mac.zip';
        }
      } else if (platform === 'win32') {
        // Windows - for future support
        assetName = 'Aster-win32-x64.zip';
      } else {
        // Linux - for future support
        assetName = `Aster-linux-${arch}.zip`;
      }
      ...
      const asset = release.assets.find((a) => a.name.toLowerCase() === assetName.toLowerCase()); // keeping comparison to lower case becasue Aster vs aster
      if (asset) {
  ...
  async downloadUpdate(
    ...
    try {
      ...
      const fileName = `aster-${latestVersion}.zip`;
