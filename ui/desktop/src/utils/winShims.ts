...
/**
 * Ensures Windows shims are available in %LOCALAPPDATA%\Aster\bin
 * This allows the bundled executables to be found via PATH regardless of where Aster is installed
 */
export async function ensureWinShims(): Promise<void> {
  ...
  try {
    ...
    // Copy command-line tools, NOT asterd.exe (which should always be used locally)
    ...
    if (!currentPath.toLowerCase().includes(tgtDir.toLowerCase())) {
      ...
      log.info(`Added ${tgtDir} to PATH for Aster processes only`);
    } else {
      ...
      if (binDirIndex > 0) {
        ...
        log.info(`Moved ${tgtDir} to beginning of PATH for Aster processes only`);
      }
    }
  } catch (error) {
