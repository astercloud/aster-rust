 ...
 * @param url The deep link URL (aster://sessions/:shareToken)
...
export async function openSharedSessionFromDeepLink(
  ...
  try {
    if (!url.startsWith('aster://sessions/')) {
      throw new Error('Invalid URL: URL must use the aster://sessions/ scheme');
    }

    // Extract the share token from the URL
    const shareToken: string = url.replace('aster://sessions/', '');
    ...
    setView('sharedSession', {
      ...
      shareToken: url.replace('aster://sessions/', ''),
      baseUrl,
    });
