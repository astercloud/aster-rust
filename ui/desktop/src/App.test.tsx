...
vi.mock('./components/ProviderGuard', () => ({
  default: ({ children }: { children: React.ReactNode }) => {
    ...
    const hasProvider = window.electron?.getConfig()?.ASTER_DEFAULT_PROVIDER;
    if (!hasProvider) {
      return <div>Welcome to Aster!</div>;
    }
    return <>{children}</>;
  },
}));
...
vi.mock('./components/AsterhintsModal', () => ({
  AsterhintsModal: () => null,
}));
...
const mockAppConfig = {
  get: vi.fn((key: string) => {
    if (key === 'ASTER_WORKING_DIR') return '/test/dir';
    return null;
  }),
};
...
describe('App Component - Brand New State', () => {
  ...
  it('should redirect to "/" when app is brand new (no provider configured)', async () => {
    ...
    mockElectron.getConfig.mockReturnValue({
      ASTER_DEFAULT_PROVIDER: null,
      ASTER_DEFAULT_MODEL: null,
      ASTER_ALLOWLIST_WARNING: false,
    });
  ...
  it('should not redirect to /welcome when provider is configured', async () => {
    ...
    mockElectron.getConfig.mockReturnValue({
      ASTER_DEFAULT_PROVIDER: 'openai',
      ASTER_DEFAULT_MODEL: 'gpt-4',
      ASTER_ALLOWLIST_WARNING: false,
    });
