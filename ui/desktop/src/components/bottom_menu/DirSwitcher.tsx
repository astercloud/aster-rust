...
export const DirSwitcher: React.FC<DirSwitcherProps> = ({ className = '' }) => {
  ...
  const handleDirectoryClick = async (event: React.MouseEvent) => {
    ...
    if (isCmdOrCtrlClick) {
      event.preventDefault();
      event.stopPropagation();
      const workingDir = window.appConfig.get('ASTER_WORKING_DIR') as string;
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            <div className="max-w-[200px] truncate [direction:rtl]">
              {String(window.appConfig.get('ASTER_WORKING_DIR'))}
            </div>
          </button>
        </TooltipTrigger>
        <TooltipContent side="top">
          {window.appConfig.get('ASTER_WORKING_DIR') as string}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
