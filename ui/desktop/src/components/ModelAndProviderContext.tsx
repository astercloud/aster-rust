...
export const ModelAndProviderProvider: React.FC<ModelAndProviderProviderProps> = ({ children }) => {
  ...
  const getFallbackModelAndProvider = useCallback(async () => {
    const provider = window.appConfig.get('ASTER_DEFAULT_PROVIDER') as string;
    const model = window.appConfig.get('ASTER_DEFAULT_MODEL') as string;
    if (provider && model) {
      try {
        await setConfigProvider({
          body: {
  ...
  const getCurrentModelAndProvider = useCallback(async () => {
    ...
    // read from config
    try {
      model = (await read('ASTER_MODEL', false)) as string;
      provider = (await read('ASTER_PROVIDER', false)) as string;
    } catch {
      console.error(`Failed to read ASTER_MODEL or ASTER_PROVIDER from config`);
      throw new Error('Failed to read ASTER_MODEL or ASTER_PROVIDER from config');
    }
    if (!model || !provider) {
  ...
  const getCurrentModelAndProviderForDisplay = useCallback(async () => {
    ...
    const asterModel = modelProvider.model;
    const asterProvider = modelProvider.provider;

    // lookup display name
    let metadata: ProviderMetadata;

    try {
      metadata = await getProviderMetadata(String(asterProvider), getProviders);
    } catch {
      return { model: asterModel, provider: asterProvider };
    }
    const providerDisplayName = metadata.display_name;

    return { model: asterModel, provider: providerDisplayName };
  }, [getCurrentModelAndProvider, getProviders]);

  const getCurrentModelDisplayName = useCallback(async () => {
    try {
      const currentModelName = (await read('ASTER_MODEL', false)) as string;
