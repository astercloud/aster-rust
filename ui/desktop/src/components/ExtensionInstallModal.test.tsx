...
describe('ExtensionInstallModal', () => {
  ...
  describe('Extension Request Handling', () => {
    it('should handle trusted extension (default behaviour, no allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'aster://extension?cmd=npx&arg=test-extension&name=TestExt');
      });
    ...
    it('should handle trusted extension (from allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'aster://extension?cmd=npx&arg=test-extension&name=AllowedExt');
      });
    ...
    it('should handle warning mode', async () => {
      ...
      await act(async () => {
        await eventHandler(
          {},
          'aster://extension?cmd=npx&arg=untrusted-extension&name=UntrustedExt'
        );
      });
    ...
    it('should handle i-ching-mcp-server as allowed command', async () => {
      ...
      await act(async () => {
        await eventHandler(
          {},
          'aster://extension?cmd=i-ching-mcp-server&id=i-ching&name=I%20Ching&description=I%20Ching%20divination'
        );
      });
    ...
    it('should handle blocked extension', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'aster://extension?cmd=npx&arg=blocked-extension&name=BlockedExt');
      });
  ...
  describe('Modal Actions', () => {
    it('should dismiss modal correctly', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'aster://extension?cmd=npx&arg=test&name=Test');
      });
    ...
    it('should handle successful extension installation', async () => {
      ...
      expect(addExtensionFromDeepLink).toHaveBeenCalledWith(
        'aster://extension?cmd=npx&arg=test&name=Test',
        mockAddExtension,
        expect.any(Function)
      );
