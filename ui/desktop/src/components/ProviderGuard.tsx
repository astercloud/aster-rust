...
import WelcomeAsterLogo from './WelcomeAsterLogo';
...
import { Aster, OpenRouter, Tetrate } from './icons';
...
export default function ProviderGuard({ didSelectProvider, children }: ProviderGuardProps) {
  ...
  const handleApiKeySuccess = async (provider: string, _model: string, apiKey: string) => {
    ...
    await upsert('ASTER_PROVIDER', provider, false);
  ...
  useEffect(() => {
    const checkProvider = async () => {
      try {
        const provider = ((await read('ASTER_PROVIDER', false)) as string) || '';
            ...
            <div className="max-w-2xl w-full mx-auto p-8">
              ...
              <div className="text-left mb-8 sm:mb-12">
                <div className="space-y-3 sm:space-y-4">
                  <div className="origin-bottom-left aster-icon-animation">
                    <Aster className="size-6 sm:size-8" />
                  </div>
                  <h1 className="text-2xl sm:text-4xl font-light text-left">Welcome to Aster</h1>
                </div>
                <p className="text-text-muted text-base sm:text-lg mt-4 sm:mt-6">
                  Since it’s your first time here, let’s get you set up with an AI provider so aster
                  can work its magic.
                </p>
              </div>
