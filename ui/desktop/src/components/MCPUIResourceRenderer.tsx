  ...
  useEffect(() => {
    const fetchProxyUrl = async () => {
      try {
        ...
        if (asterApiHost && secretKey) {
          setProxyUrl(`${asterApiHost}/mcp-ui-proxy?secret=${encodeURIComponent(secretKey)}`);
        } else {
          console.error('Failed to get asterd host/port or secret key');
        }
      } catch (error) {
    ...
    <div className="mt-3 p-4 border border-borderSubtle rounded-lg bg-background-muted">
      <div className="overflow-hidden rounded-sm">
        <UIResourceRenderer
          ...
          supportedContentTypes={['rawHtml', 'externalUrl']} // Aster does not support remoteDom content
          htmlProps={{
            autoResizeIframe: {
              height: true,
