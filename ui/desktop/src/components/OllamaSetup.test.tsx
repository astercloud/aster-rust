...
describe('OllamaSetup', () => {
  ...
  describe.skip('when Ollama and model are both available', () => {
    ...
    it('should handle successful connection', async () => {
      ...
      await waitFor(() => {
        fireEvent.click(screen.getByText(/Use Aster with Ollama/));
      });

      await waitFor(() => {
        expect(mockUpsert).toHaveBeenCalledWith('ASTER_PROVIDER', 'ollama', false);
        expect(mockUpsert).toHaveBeenCalledWith('ASTER_MODEL', 'gpt-oss:20b', false);
