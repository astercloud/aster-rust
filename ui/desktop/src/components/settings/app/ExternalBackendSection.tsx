...
interface ExternalAsterdConfig {
  enabled: boolean;
  url: string;
  secret: string;
}
...
interface Settings {
  externalAsterd?: Partial<ExternalAsterdConfig>;
}

const DEFAULT_CONFIG: ExternalAsterdConfig = {
  enabled: false,
  url: '',
  secret: '',
};

function parseConfig(partial: Partial<ExternalAsterdConfig> | undefined): ExternalAsterdConfig {
  return {
...
export default function ExternalBackendSection() {
  const [config, setConfig] = useState<ExternalAsterdConfig>(DEFAULT_CONFIG);
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      setConfig(parseConfig(settings?.externalAsterd));
    };
    loadSettings();
  }, []);
  ...
  const saveConfig = async (newConfig: ExternalAsterdConfig): Promise<void> => {
    setIsSaving(true);
    try {
      const currentSettings = ((await window.electron.getSettings()) as Settings) || {};
      await window.electron.saveSettings({
        ...currentSettings,
        externalAsterd: newConfig,
      });
    } catch (error) {
  ...
  const updateField = <K extends keyof ExternalAsterdConfig>(
    field: K,
    value: ExternalAsterdConfig[K]
  ) => {
    ...
    <section id="external-backend" className="space-y-4 pr-4 mt-1">
      <Card className="pb-2">
        <CardHeader className="pb-0">
          <CardTitle>Aster Server</CardTitle>
          <CardDescription>
            By default aster launches a server for you, use this to connect to an external aster
            server
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-text-muted max-w-md mt-[2px]">
                Connect to a aster server running elsewhere (requires app restart)
              </p>
            </div>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-text-muted">
                  The secret key configured on the asterd server (ASTER_SERVER__SECRET_KEY)
                </p>
              </div>

              <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md p-3">
                <p className="text-xs text-amber-800 dark:text-amber-200">
                  <strong>Note:</strong> Changes require restarting Aster to take effect. New chat
