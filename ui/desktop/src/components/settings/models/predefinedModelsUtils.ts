...
export function getPredefinedModelsFromEnv(): Model[] {
  try {
    const envModels = window.appConfig.get('ASTER_PREDEFINED_MODELS'); // process.env.ASTER_PREDEFINED_MODELS
    if (envModels && typeof envModels === 'string') {
      return JSON.parse(envModels) as Model[];
    }
  } catch (error) {
    console.warn('Failed to parse ASTER_PREDEFINED_MODELS environment variable:', error);
  }
  return [];
}
