    ...
    const loadConfig = async () => {
      try {
        ...
        const [
          ...
          read('ASTER_LEAD_MODEL', false),
          read('ASTER_LEAD_PROVIDER', false),
          read('ASTER_LEAD_TURNS', false),
          read('ASTER_LEAD_FAILURE_THRESHOLD', false),
          read('ASTER_LEAD_FALLBACK_TURNS', false),
        ]);
        ...
        const workerModelConfig = await read('ASTER_MODEL', false);
        ...
        const workerProviderConfig = await read('ASTER_PROVIDER', false);
  ...
  const handleSave = async () => {
    try {
      if (isEnabled && leadModel && workerModel) {
        ...
        await Promise.all([
          upsert('ASTER_LEAD_MODEL', leadModel, false),
          leadProvider && upsert('ASTER_LEAD_PROVIDER', leadProvider, false),
          upsert('ASTER_MODEL', workerModel, false),
          workerProvider && upsert('ASTER_PROVIDER', workerProvider, false),
          upsert('ASTER_LEAD_TURNS', leadTurns, false),
          upsert('ASTER_LEAD_FAILURE_THRESHOLD', failureThreshold, false),
          upsert('ASTER_LEAD_FALLBACK_TURNS', fallbackTurns, false),
        ]);
      } else {
        // Remove lead/worker configuration
        await Promise.all([
          remove('ASTER_LEAD_MODEL', false),
          remove('ASTER_LEAD_PROVIDER', false),
          remove('ASTER_LEAD_TURNS', false),
          remove('ASTER_LEAD_FAILURE_THRESHOLD', false),
          remove('ASTER_LEAD_FALLBACK_TURNS', false),
        ]);
      }
      onClose();
    } catch (error) {
