...
describe('LeadWorkerSettings', () => {
  ...
  const setupHappyPathMocks = () => {
    ...
    mockRead.mockImplementation(async (key: string) => {
      switch (key) {
        case 'ASTER_LEAD_MODEL':
          return 'my-custom-lead';
        case 'ASTER_LEAD_PROVIDER':
          return 'anthropic';
        case 'ASTER_LEAD_TURNS':
          return 3;
        case 'ASTER_LEAD_FAILURE_THRESHOLD':
          return 2;
        case 'ASTER_LEAD_FALLBACK_TURNS':
          return 2;
        case 'ASTER_MODEL':
  ...
  it('shows custom inputs for lead/worker when current models are unknown and saves them', async () => {
    ...
    await waitFor(() => {
      expect(mockUpsert).toHaveBeenCalledWith('ASTER_LEAD_MODEL', 'my-custom-lead', false);
      expect(mockUpsert).toHaveBeenCalledWith('ASTER_MODEL', 'my-custom-worker', false);
      expect(mockUpsert).toHaveBeenCalledWith('ASTER_LEAD_PROVIDER', 'anthropic', false);
      expect(mockUpsert).toHaveBeenCalledWith('ASTER_PROVIDER', 'openai', false);
    });
  });
  ...
  it('disables lead/worker and removes config when toggled off', async () => {
    ...
    await waitFor(() => {
      expect(mockRemove).toHaveBeenCalledWith('ASTER_LEAD_MODEL', false);
      expect(mockRemove).toHaveBeenCalledWith('ASTER_LEAD_PROVIDER', false);
      expect(mockRemove).toHaveBeenCalledWith('ASTER_LEAD_TURNS', false);
      expect(mockRemove).toHaveBeenCalledWith('ASTER_LEAD_FAILURE_THRESHOLD', false);
      expect(mockRemove).toHaveBeenCalledWith('ASTER_LEAD_FALLBACK_TURNS', false);
    });
  });
});
