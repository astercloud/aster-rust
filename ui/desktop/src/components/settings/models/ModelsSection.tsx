  ...
  const loadModelData = useCallback(async () => {
    try {
      ...
      if (providerDisplayName) {
        ...
        const asterProvider = (await read('ASTER_PROVIDER', false)) as string;
        const providers = await getProviders(true);
        const providerDetailsList = providers.filter((provider) => provider.name === asterProvider);
