...
import { all_aster_modes, ModeSelectionItem } from './ModeSelectionItem';
...
export const ModeSection = () => {
  ...
  const fetchMaxTurns = useCallback(async () => {
    try {
      const turns = (await read('ASTER_MAX_TURNS', false)) as number;
      if (turns) {
        setMaxTurns(turns);
      }
    } catch (error) {
  ...
  const handleMaxTurnsChange = async (value: number) => {
    try {
      await upsert('ASTER_MAX_TURNS', value, false);
      setMaxTurns(value);
    } catch (error) {
