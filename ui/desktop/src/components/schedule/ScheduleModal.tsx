...
async function parseDeepLink(deepLink: string): Promise<Recipe | null> {
  try {
    ...
    if (url.protocol !== 'aster:' || (url.hostname !== 'bot' && url.hostname !== 'recipe')) {
      return null;
    }
      ...
      try {
        ...
        if (recipe) {
          ...
          setInternalValidationError(
            'Invalid deep link format. Please use a aster://bot or aster://recipe link.'
          );
        }
      } catch {
        setParsedRecipe(null);
        setInternalValidationError(
          'Failed to parse deep link. Please ensure using a aster://bot or aster://recipe link and try again.'
        );
      }
    } else {
      setParsedRecipe(null);
  ...
  const handleLocalSubmit = async (event: FormEvent) => {
    ...
    if (sourceType === 'file') {
      ...
      try {
        ...
        const tempDir = window.electron.getConfig().ASTER_WORKING_DIR || '.';
                  ...
                  {sourceType === 'deeplink' && (
                    <div>
                      <Input
                        ...
                        placeholder="Paste aster://bot or aster://recipe link here..."
