...
const importRecipeSchema = z
  .object({
    deeplink: z
      .string()
      .refine(
        (value) => !value || value.trim().startsWith('aster://recipe?config='),
        'Invalid deeplink format. Expected: aster://recipe?config=...'
      ),
    recipeUploadFile: z
      .instanceof(File)
      .nullable()
...
export default function ImportRecipeForm({ isOpen, onClose, onSuccess }: ImportRecipeFormProps) {
  ...
  const parseDeeplink = async (deeplink: string): Promise<Recipe | null> => {
    try {
      ...
      if (!cleanLink.startsWith('aster://recipe?config=')) {
        throw new Error('Invalid deeplink format. Expected: aster://recipe?config=...');
      }

      const recipeEncoded = cleanLink.replace('aster://recipe?config=', '');
          ...
          >
            <div className="space-y-4">
                        ...
                        return (
                          <div className={isDisabled ? 'opacity-50' : ''}>
                            ...
                            <textarea
                              ...
                              placeholder="Paste your aster://recipe?config=... deeplink here"
                              rows={3}
                            ...
                            >
                              Paste a recipe deeplink starting with "aster://recipe?config="
                            </p>
                            {field.state.meta.errors.length > 0 && (
              ...
              <p className="text-xs text-text-muted">
                Ensure you review contents of recipe files before adding them to your aster
                interface.
              </p>
            </div>
