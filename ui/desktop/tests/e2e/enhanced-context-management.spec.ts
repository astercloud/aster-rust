...
test.describe('Enhanced Context Management E2E Tests', () => {
  ...
  test.describe('Context Window Alert System', () => {
    test('should show context window alert only when tokens are being used', async () => {
      ...
      await page.waitForSelector('[data-testid="loading-aster"]', { state: 'hidden', timeout: 30000 });
  ...
  test.describe('Manual Compaction Workflow', () => {
    test('should perform complete manual compaction workflow', async () => {
      ...
      const loadingAster = page.locator('[data-testid="loading-aster"]');
      await expect(loadingAster).toBeVisible();
      await expect(loadingAster).toContainText('aster is compacting the conversation...');
  ...
  test.describe('Post-Compaction Behavior', () => {
    ...
    test('should continue conversation normally after compaction', async () => {
      ...
      await expect(page.locator('[data-testid="loading-aster"]')).toBeVisible();
      await page.waitForSelector('[data-testid="loading-aster"]', { state: 'hidden', timeout: 30000 });
  ...
  test.describe('Error Handling', () => {
    ...
    test('should handle network timeouts during compaction', async () => {
      ...
      await page.waitForSelector('[data-testid="loading-aster"]', { state: 'hidden', timeout: 35000 });
  ...
  test.describe('UI State Management', () => {
    ...
    test('should show appropriate loading states', async () => {
      ...
      // Verify loading aster appears with correct message
      const loadingAster = page.locator('[data-testid="loading-aster"]');
      await expect(loadingAster).toBeVisible();
      await expect(loadingAster).toContainText('aster is compacting the conversation...');
      
      // Verify no other loading indicators are shown
      const regularLoadingMessages = page.locator('[data-testid="loading-aster"]:not(:has-text("compacting"))');
      ...
      await expect(loadingAster).not.toBeVisible();
    });
  });
  ...
  test.describe('Performance and Reliability', () => {
    test('should handle large conversations efficiently', async () => {
      ...
      await page.waitForSelector('[data-testid="loading-aster"]', { state: 'hidden', timeout: 45000 });
