    ...
    test.describe(`Provider: ${provider.name}`, () => {
      ...
      test.describe('Chat', () => {
        test('chat interaction', async () => {
          ...
          const loadingAster = await mainWindow.waitForSelector('[data-testid="loading-indicator"]',
            { timeout: 2000 });
          expect(await loadingAster.isVisible()).toBe(true);
      ...
      test.describe('MCP Integration', () => {
        ...
        test('test running quotes functionality', async () => {
          ...
          // Get the latest response
          const response = await mainWindow.waitForSelector('.aster-message-tool', { timeout: 5000 });
          expect(await response.isVisible()).toBe(true);
