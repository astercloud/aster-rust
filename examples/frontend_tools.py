...
aster_PORT = "3001"
aster_URL = f"http://{aster_HOST}:{aster_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{aster_URL}/agent/update_provider",
            json={"provider": "databricks", "model": "aster"},
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to aster.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with aster."""
