...
async fn start_agent(
    ...
    aster::posthog::set_session_context("desktop", false);
    ...
    let original_recipe = if let Some(deeplink) = recipe_deeplink {
        match recipe_deeplink::decode(&deeplink) {
            ...
            Err(err) => {
                ...
                aster::posthog::emit_error("recipe_deeplink_decode_failed", &err.to_string());
    ...
    let mut session =
        SessionManager::create_session(PathBuf::from(&working_dir), name, SessionType::User)
            ...
            .map_err(|err| {
                ...
                aster::posthog::emit_error("session_create_failed", &err.to_string());
                ErrorResponse {
    ...
    if payload.load_model_and_extensions {
        ...
        let provider_result = async {
            let provider_name = session
                .provider_name
                .clone()
                .or_else(|| config.get_aster_provider().ok())
            ...
            let model_config = match session.model_config.clone() {
                ...
                None => {
                    let model_name = config.get_aster_model().map_err(|_| ErrorResponse {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_aster_model().ok()) {
        Some(m) => m,
        None => {
