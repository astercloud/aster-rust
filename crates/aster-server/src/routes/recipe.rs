    ...
    match recipe_result {
        Ok(mut recipe) => {
            if let Some(author_req) = request.author {
                recipe.author = Some(aster::recipe::Author {
        ...
        Err(e) => {
            ...
            aster::posthog::emit_error("recipe_create_failed", &e.to_string());
            let error_response = CreateRecipeResponse {
                recipe: None,
...
async fn encode_recipe(
    ...
    match recipe_deeplink::encode(&request.recipe) {
        ...
        Err(err) => {
            ...
            aster::posthog::emit_error("recipe_encode_failed", &err.to_string());
...
async fn decode_recipe(
    ...
    match recipe_deeplink::decode(&request.deeplink) {
        ...
        Err(err) => {
            ...
            aster::posthog::emit_error("recipe_decode_failed", &err.to_string());
            Err(StatusCode::BAD_REQUEST)
        }
    }
}
...
async fn schedule_recipe(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            aster::posthog::emit_error("recipe_schedule_failed", &e.to_string());
