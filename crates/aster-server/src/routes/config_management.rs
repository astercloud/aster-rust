...
use aster::config::{Config, ConfigError};
use aster::model::ModelConfig;
use aster::providers::auto_detect::detect_provider_from_api_key;
use aster::providers::base::{ProviderMetadata, ProviderType};
use aster::providers::canonical::maybe_get_canonical_model;
use aster::providers::create_with_default_model;
use aster::providers::errors::ProviderError;
use aster::providers::providers as get_providers;
use aster::providers::{retry_operation, RetryConfig};
use aster::{
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, StatusCode> {
    let extensions = aster::config::get_all_extensions();
    let warnings = aster::config::get_warnings();
...
pub async fn add_extension(
    ...
    let extensions = aster::config::get_all_extensions();
    let key = aster::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    aster::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, StatusCode> {
    let key = aster::config::extensions::name_to_key(&name);
    aster::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let loaded_provider = aster::config::declarative_providers::load_provider(name.as_str()).ok();
    ...
    let provider = aster::providers::create(&name, model_config)
        .await
...
pub async fn init_config() -> Result<Json<String>, StatusCode> {
    ...
    match aster::config::base::load_init_config_from_workspace() {
...
pub async fn upsert_permissions(
    ...
    let mut permission_manager = aster::config::PermissionManager::default();
...
pub async fn create_custom_provider(
    ...
    let config = aster::config::declarative_providers::create_custom_provider(
        &request.engine,
    ...
    if let Err(e) = aster::providers::refresh_custom_providers().await {
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = aster::config::declarative_providers::load_provider(id.as_str())
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, StatusCode> {
    aster::config::declarative_providers::remove_custom_provider(&id)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn update_custom_provider(
    ...
    aster::config::declarative_providers::update_custom_provider(
        &id,
        &request.engine,
        request.display_name,
        request.api_url,
...
pub async fn set_config_provider(
    ...
    create_with_default_model(&provider)
        ...
        .and_then(|_| {
            let config = Config::global();
            config
                .set_aster_provider(provider)
                .and_then(|_| config.set_aster_model(model))
...
mod tests {
    ...
    async fn test_read_model_limits() {
        ...
        let limits: Vec<aster::model::ModelLimitConfig> = serde_json::from_value(response).unwrap();
