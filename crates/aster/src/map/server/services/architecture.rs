...
use crate::map::server::types::{
    ArchitectureMap, LogicBlock, LogicBlockType,
...
pub fn build_architecture_map(blueprint: &EnhancedCodeBlueprint) -> ArchitectureMap {
    ...
    for (dir, mods) in &dir_groups {
        ...
        let description = if !descriptions.is_empty() {
            ...
            let func_names: Vec<String> = mods
                .iter()
                .take(5)
                .map(|m| m.name.trim_end_matches(".ts").trim_end_matches(".js").to_string())
                .collect();
    ...
    let project_desc = blueprint.project.semantic
        .as_ref()
...
pub fn get_module_detail(
    ...
    for symbol in blueprint.symbols.values() {
        ...
        let info = SymbolInfo {
            ...
            semantic: symbol.semantic.as_ref().map(|s| {
    ...
    Some(ModuleDetailInfo {
        ...
        lines: module.lines,
        semantic: module.semantic.as_ref().map(|s| {
            serde_json::to_value(s).unwrap_or_default()
        }),
        symbols,
        ...
        if call.callee == symbol_id {
            ...
            refs.called_by.push(CallerInfo {
                ...
                symbol_name: caller_symbol
                    .map(|s| s.name.clone())
                    .unwrap_or_else(|| {
                        call.caller.split("::").last().unwrap_or("").to_string()
                    }),
                module_id: caller_symbol
                    .map(|s| s.module_id.clone())
                    .unwrap_or_default(),
                call_type: call.call_type.clone(),
                locations: call.locations.iter()
                    .map(|loc| LineLocation { line: loc.start_line as usize })
                    .collect(),
            });
        }
        ...
        if call.caller == symbol_id {
            ...
            refs.calls.push(CallerInfo {
                ...
                symbol_name: callee_symbol
                    .map(|s| s.name.clone())
                    .unwrap_or_else(|| {
                        call.callee.split("::").last().unwrap_or("").to_string()
                    }),
    ...
    for type_ref in &blueprint.references.type_refs {
        if type_ref.child == symbol_id {
            ...
            refs.type_refs.push(TypeRefInfo {
                ...
                related_symbol_name: parent_symbol
                    .map(|s| s.name.clone())
                    .unwrap_or_default(),
        ...
        if type_ref.parent == symbol_id {
            ...
            refs.type_refs.push(TypeRefInfo {
                ...
                related_symbol_name: child_symbol
                    .map(|s| s.name.clone())
                    .unwrap_or_default(),
                kind: format!("{:?}", type_ref.kind).to_lowercase(),
