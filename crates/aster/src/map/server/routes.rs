...
impl ApiError {
    pub fn not_found(msg: &str) -> Self {
        Self { message: msg.to_string(), status_code: 404 }
    }
    
    pub fn bad_request(msg: &str) -> Self {
        Self { message: msg.to_string(), status_code: 400 }
    }
    
    pub fn internal(msg: &str) -> Self {
        Self { message: msg.to_string(), status_code: 500 }
    }
}
...
pub fn load_enhanced_blueprint(ontology_path: &Path) -> Result<EnhancedCodeBlueprint, ApiError> {
    let content = fs::read_to_string(ontology_path)
        .map_err(|e| ApiError::internal(&format!("读取文件失败: {}", e)))?;
    serde_json::from_str(&content)
        .map_err(|e| ApiError::internal(&format!("解析蓝图失败: {}", e)))
}

/// 推断 map 目录
pub fn infer_map_dir(ontology_path: &Path) -> PathBuf {
    if ontology_path.extension().map(|e| e == "json").unwrap_or(false) {
...
impl ApiHandlers {
    ...
    pub fn get_ontology(&self) -> Result<serde_json::Value, ApiError> {
        ...
        if index_path.exists() {
            ...
            Err(ApiError::not_found("Blueprint not found. Please run /map generate first."))
        }
    }
    ...
    pub fn get_chunk(&self, chunk_path: &str) -> Result<serde_json::Value, ApiError> {
        ...
        let chunk_file = self.map_dir.join("chunks").join(format!("{}.json", chunk_path));
        if !chunk_file.exists() {
            return Err(ApiError::not_found(&format!("Chunk not found: {}", chunk_path)));
        }
    ...
    pub fn get_entry_points(&self) -> Result<EntryPointsResponse, ApiError> {
        ...
        Ok(EntryPointsResponse { entry_points: entries })
    }
        ...
        for module in blueprint.modules.values() {
                ...
                || module.id.to_lowercase().contains(&query_lower)
            {
                results.push(SearchResultItem {
                    ...
                    module_id: None,
                    description: module.semantic.as_ref()
        ...
        for symbol in blueprint.symbols.values() {
            if symbol.name.to_lowercase().contains(&query_lower) {
                ...
                results.push(SearchResultItem {
                    ...
                    description: symbol.semantic.as_ref()
                        .map(|s| s.description.clone()),
                });
            }
        }
            ...
            if path.extension().map(|e| e == "json").unwrap_or(false) {
                if let Some(file_name) = path.file_stem().and_then(|s| s.to_str()) {
                    ...
                    if let Ok(meta) = fs::metadata(&path) {
                        ...
                        metadata.insert(dir_path, ChunkMetadata {
