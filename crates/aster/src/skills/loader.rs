        ...
        for line in frontmatter_text.lines() {
            if let Some(colon_idx) = line.find(':') {
                ...
                match key {
                    ...
                    "disable-model-invocation" => frontmatter.disable_model_invocation = Some(clean_value),
                    _ => { extra_fields.insert(key.to_string(), clean_value); }
                }
            }
        }
...
pub fn load_skill_from_file(
    ...
    let display_name = frontmatter.name.clone().unwrap_or_else(|| skill_name.to_string());
    ...
    let disable_model_invocation = parse_boolean(frontmatter.disable_model_invocation.as_deref(), false);
        ...
        for plugin_entry in plugins.flatten() {
            ...
            for version_entry in versions.flatten() {
                ...
                for skill_dir_entry in skill_dirs.flatten() {
                    ...
                    if let Ok(skill) = load_skill_from_file(&skill_name, &skill_md_path, SkillSource::Plugin) {
                        results.push(skill);
                    }
                }
            }
...
"#;
    ...
    fn test_parse_allowed_tools() {
        ...
        assert_eq!(
            ...
            Some(vec!["tool1".to_string(), "tool2".to_string(), "tool3".to_string()])
        );
    }
...
"#).unwrap();
        ...
        assert_eq!(skill.allowed_tools, Some(vec!["tool1".to_string(), "tool2".to_string()]));
