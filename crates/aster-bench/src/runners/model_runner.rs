...
impl ModelRunner {
    ...
    fn run_benchmark(
        ...
        envs.push(("ASTER_MODEL".to_string(), model.clone().name));
        envs.push(("ASTER_PROVIDER".to_string(), model.clone().provider));
        ...
        if let Some(model) = self.config.models.first() {
            if let Some(shim_opt) = &model.tool_shim {
                if shim_opt.use_tool_shim {
                    shim_envs.push(("ASTER_TOOLSHIM".to_string(), "true".to_string()));
