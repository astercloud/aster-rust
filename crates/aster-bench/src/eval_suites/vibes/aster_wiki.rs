...
impl AsterWiki {
    ...
    fn check_html_implementation(&self, content: &str) -> bool {
        ...
        let has_wiki_elements = content.contains("<h1") && // Has headings
                              ...
                              content.contains("aster") && // Mentions aster
...
impl Evaluation for AsterWiki {
    async fn run(
        ...
        let (messages, perf_metrics) = collect_baseline_metrics(
            agent,
            "Create a Wikipedia-style web page about aster (Block's AI agent) in a new index.html file. The page should be a complete, well-structured HTML document with proper head and body sections. Use heading tags (h1, h2, h3) to organize the content into clear sections. Include comprehensive information about aster organized in a way similar to how Wikipedia presents technical topics. Remember to use your tools if applicable.".to_string()
        ).await;
