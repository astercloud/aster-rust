...
use aster::tracing::{langfuse_layer, otlp_layer};
use aster_bench::bench_session::BenchAgentError;
use aster_bench::error_capture::ErrorCaptureLayer;
...
fn setup_logging_internal(
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = aster::logging::prepare_log_directory("cli", true)?;
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = aster::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        // Verify directory structure
        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "aster"));
