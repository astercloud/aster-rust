...
use aster::providers::{create, testprovider::TestProvider};
...
{
    use aster::config::ExtensionConfig;
    use tokio::sync::Mutex;

    aster::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn aster::providers::base::Provider>,
            &session.id,
        )
        .await?;
