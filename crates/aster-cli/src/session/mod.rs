...
impl CliSession {
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let config = ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: aster::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            ...
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    if curr_aster_mode != AsterMode::Auto {
                        config.set_aster_mode(AsterMode::Auto).unwrap();
                    }
                ...
                result = stream.next() => {
                    match result {
                                ...
                                for content in &message.content {
                                    if let MessageContent::ToolRequest(tool_request) = content {
                                        if let Ok(tool_call) = &tool_request.tool_call {
                                            tracing::info!(counter.aster.tool_calls = 1,
                        ...
                        Some(Err(e)) => {
                            ...
                            if e.downcast_ref::<aster::providers::errors::ProviderError>()
                                .map(|provider_error| matches!(provider_error, aster::providers::errors::ProviderError::ContextLengthExceeded(_)))
                                .unwrap_or(false) {
    ...
    pub async fn get_session(&self) -> Result<aster::session::Session> {
