...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn aster::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    aster::posthog::set_session_context("cli", session_config.resume);
    ...
    let provider_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.aster_provider.clone())
        })
    ...
    let model_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.aster_model.clone())
        })
        .or_else(|| config.get_aster_model().ok())
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        aster::model::ModelConfig::new(&model_name)
            .unwrap_or_else(|e| {
