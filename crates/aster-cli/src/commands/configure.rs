...
use aster::config::declarative_providers::{create_custom_provider, remove_custom_provider};
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &aster::providers::base::ProviderMetadata,
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_aster_provider().ok();
    ...
    {
        Ok(()) => {
            config.set_aster_provider(provider_name)?;
            config.set_aster_model(&model)?;
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&aster::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_goose_mode_dialog() -> anyhow::Result<()> {
    ...
    let msg = match mode {
        AsterMode::Auto => "Set to Auto Mode - full file modification enabled",
        AsterMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        AsterMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        AsterMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    let _ = cliclack::log::warning("Note: Disabling the keyring stores secrets in a plain text file (~/.config/aster/secrets.yaml)");
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use aster::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use aster::conversation::message::Message;
    ...
    let configured_model: String = config.get_aster_model()?;
    let model_config = match aster::model::ModelConfig::new(&configured_model) {
        Ok(config) => config,
        Err(e) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Builtin {
                                ...
                                display_name: Some(aster::config::DEFAULT_DISPLAY_NAME.to_string()),
...
fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = aster::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        aster::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
