...
pub async fn handle_web(
    ...
    crate::logging::setup_logging(Some("aster-web"), None)?;

    let config = aster::config::Config::global();

    let provider_name: String = match config.get_aster_provider() {
        Ok(p) => p,
        Err(_) => {
            eprintln!("No provider configured. Run 'aster configure' first");
            std::process::exit(1);
        }
    };

    let model: String = match config.get_aster_model() {
        Ok(m) => m,
        Err(_) => {
            eprintln!("No model configured. Run 'aster configure' first");
            std::process::exit(1);
        }
    };

    let model_config = aster::model::ModelConfig::new(&model)?;
    ...
    let enabled_configs = aster::config::get_enabled_extensions();
...
async fn process_message_streaming(
    ...
    if provider.is_err() {
        let error_msg = "I'm not properly configured yet. Please configure a provider through the CLI first using `aster configure`.".to_string();
        let mut sender = sender.lock().await;
        let _ = sender
            .send(Message::Text(
