...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("aster");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run aster. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("aster Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run aster
            let mut command = std::process::Command::new("aster");
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run aster with a fresh session
            let mut command = std::process::Command::new("aster");
            command.arg("session");
