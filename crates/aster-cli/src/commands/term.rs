...
if [[ -z "$aster_preexec_installed" ]]; then
    aster_preexec_installed=1
    trap 'aster_preexec "$BASH_COMMAND"' DEBUG
...
add-zsh-hook preexec aster_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function aster_preexec --on-event fish_preexec
    string match -q -r '^aster term' -- $argv[1]; and return
    string match -q -r '^(@aster|@g)($|\s)' -- $argv[1]; and return
    {aster_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    ...
    if ($line -notmatch '^aster term' -and $line -notmatch '^(@aster|@g)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{aster_bin}' term log $using:line }} | Out-Null
    }}
...
pub async fn handle_term_init(
    ...
    let aster_bin = std::env::current_exe()
    ...
    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|s| s.replace("{aster_bin}", &aster_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
...
pub async fn handle_term_info() -> Result<()> {
    ...
    let config = aster::config::Config::global();
    let model_name = config
        .get_aster_model()
        .ok()
        .map(|name| {
            let short = name.rsplit('/').next().unwrap_or(&name);
            if let Some(stripped) = short.strip_prefix("aster-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = config
        .get_aster_model()
        .ok()
        .and_then(|model_name| aster::model::ModelConfig::new(&model_name).ok())
