use anyhow::Result;
use aster::agents::extension::{Envs, PlatformExtensionContext, PLATFORM_EXTENSIONS};
use aster::agents::{Agent, ExtensionConfig, SessionConfig};
use aster::config::{get_all_extensions, Config};
use aster::conversation::message::{ActionRequiredData, Message, MessageContent};
use aster::conversation::Conversation;
use aster::mcp_utils::ToolResult;
use aster::permission::permission_confirmation::PrincipalType;
use aster::permission::{Permission, PermissionConfirmation};
...
impl AsterAcpAgent {
    async fn new(builtins: Vec<String>) -> Result<Self> {
        ...
        let model_config = aster::model::ModelConfig {
