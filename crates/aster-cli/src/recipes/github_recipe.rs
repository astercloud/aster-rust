...
use aster::recipe::template_recipe::parse_recipe_content;
use aster::recipe::RECIPE_FILE_EXTENSIONS;
use console::style;
use serde::{Deserialize, Serialize};

use aster::recipe::read_recipe_file_content::RecipeFile;
use std::env;
use std::fs;
use std::path::Path;
...
pub const ASTER_RECIPE_GITHUB_REPO_CONFIG_KEY: &str = "ASTER_RECIPE_GITHUB_REPO";
...
fn discover_github_recipes(repo: &str) -> Result<Vec<RecipeInfo>> {
    ...
    let output = Command::new("gh")
        ...
        .output()
        .map_err(|e| anyhow!("Failed to fetch repository contents using 'gh api' command (executed when ASTER_RECIPE_GITHUB_REPO is configured). This requires GitHub CLI (gh) to be installed and authenticated. Error: {}", e))?;
