...
mod tests {
    ...
    fn test_extract_recipe_info_from_cli_basic() {
        ...
        assert_eq!(settings.aster_provider, Some("test_provider".to_string()));
        assert_eq!(settings.aster_model, Some("test_model".to_string()));
...
settings:
  aster_provider: test_provider
  aster_model: test_model
  temperature: 0.7
sub_recipes:
