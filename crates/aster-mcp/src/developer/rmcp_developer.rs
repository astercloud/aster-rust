    ...
    fn get_info(&self) -> ServerInfo {
        ...
        ServerInfo {
            server_info: Implementation {
                name: "aster-developer".to_string(),
...
impl DeveloperServer {
    ...
    pub async fn text_editor(
        ...
        if self.is_ignored(&path) {
            return Err(ErrorData::new(
                ErrorCode::INTERNAL_ERROR,
                format!(
                    "Access to '{}' is restricted by .asterignore",
                    path.display()
                ),
                None,
            ));
        ...
        for arg in &cmd_parts[1..] {
            ...
            if self.is_ignored(path) {
                return Err(ErrorData::new(
                    ErrorCode::INTERNAL_ERROR,
                    format!(
                        "The command attempts to access '{}' which is restricted by .asterignore",
                        arg
                    ),
                    None,
                ));
    ...
    fn build_ignore_patterns(cwd: &PathBuf) -> Gitignore {
        ...
        let local_ignore_path = cwd.join(".asterignore");
...
mod tests {
    ...
    async fn test_aster_ignore_basic_patterns() {
        ...
        // Create .asterignore file with patterns
        fs::write(".asterignore", "secret.txt\n*.env").unwrap();
    ...
    async fn test_text_editor_respects_ignore_patterns() {
        ...
        // Create .asterignore file
        fs::write(".asterignore", "secret.txt").unwrap();
