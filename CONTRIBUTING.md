...
If you use Aster, Copilot, Claude, or other AI tools to help with your PRs:  
...
aster includes rust binaries alongside an electron app for the GUI. To work
...
First let's compile aster and try it out

```
cargo build
```

when that is done, you should now have debug builds of the binaries like the aster cli:

```
./target/debug/aster --help
```

If you haven't used the CLI before, you can use this compiled version to do first time configuration:

```
./target/debug/aster configure
```

And then once you have a connection to an LLM provider working, you can run a session!

```
./target/debug/aster session
```

These same commands can be recompiled and immediately run using `cargo run -p aster-cli` for iteration.
...
you can talk to aster!

You can now make changes in the code in ui/desktop to iterate on the GUI half of aster.

### Regenerating the OpenAPI schema

The file `ui/desktop/openapi.json` is automatically generated during the build.
It is written by the `generate_schema` binary in `crates/aster-server`.
...
Changes to the API should be made in the Rust source under `crates/aster-server/src/`.

### Debugging

To debug the Aster server, you can run it from your preferred IDE. How to configure the command
to start the server will depend on your IDE. The command to run is:

```
export ASTER_SERVER__SECRET_KEY=test
cargo run --package aster-server --bin asterd -- agent   # or: `just run-server`
```

The server will start listening on port `3000` by default, but this can be changed by setting the
`ASTER_PORT` environment variable.
...
8. Open a Pull Request from your branch on your fork to block/aster’s main branch.
...
You can change the provider aster points to via the `ASTER_PROVIDER` env var. If you already
...
When testing changes or running multiple aster configurations, use `ASTER_PATH_ROOT` to isolate your data:

```bash
# Test with a clean environment
export ASTER_PATH_ROOT="/tmp/aster-test"
./target/debug/aster session

# Or for a single command
ASTER_PATH_ROOT="/tmp/aster-dev" cargo run -p aster-cli -- session
```

This creates isolated `config/`, `data/`, and `state/` directories under the specified path, preventing your test sessions from affecting your main aster installation. See the [environment variables guide](./documentation/docs/guides/environment-variables.md#development--testing) for more details.

## Enable traces in aster with [locally hosted Langfuse](https://langfuse.com/docs/deployment/self-host)
...
- Set the environment variables so that aster can connect to the langfuse server:
...
There are numerous ways to be an open source contributor and contribute to aster. We're here to help you on your way! Here are some suggestions to get started. If you have any questions or need help, feel free to reach out to us on [Discord](https://discord.gg/aster-oss).
...
- **Spread the Word:** Help us reach more people by sharing aster's project, website, YouTube, and/or Twitter/X.
