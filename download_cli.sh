...
#   ASTER_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Overrides CANARY. Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   ASTER_PROVIDER - Optional: provider for aster
#   ASTER_MODEL    - Optional: model for aster
#   CANARY         - Optional: if set to "true", downloads from canary release instead of stable
#   CONFIGURE      - Optional: if set to "false", disables running aster configure interactively
...
if ! command -v curl >/dev/null 2>&1; then
  echo "Error: 'curl' is required to download aster. Please install curl and try again."
  exit 1
fi

# Check for tar or unzip (depending on OS)
if ! command -v tar >/dev/null 2>&1 && ! command -v unzip >/dev/null 2>&1; then
  echo "Error: Either 'tar' or 'unzip' is required to extract aster. Please install one and try again."
  exit 1
fi
...
if [[ "${WINDIR:-}" ]] || [[ "${windir:-}" ]] || [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    ...
    DEFAULT_BIN_DIR="$USERPROFILE/aster"
...
ASTER_BIN_DIR="${ASTER_BIN_DIR:-$DEFAULT_BIN_DIR}"
RELEASE="${CANARY:-false}"
CONFIGURE="${CONFIGURE:-true}"
if [ -n "${ASTER_VERSION:-}" ]; then
  # Validate the version format
  if [[ ! "$ASTER_VERSION" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$ ]]; then
    echo "[error]: invalid version '$ASTER_VERSION'."
    echo "  expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
    exit 1
  fi
  ASTER_VERSION=$(echo "$ASTER_VERSION" | sed 's/^v\{0,1\}/v/') # Ensure the version string is prefixed with 'v' if not already present
  RELEASE_TAG="$ASTER_VERSION"
else
  # If ASTER_VERSION is not set, fall back to existing behavior for backwards compatibility
...
case "$OS" in
  ...
  mingw*|msys*|cygwin*)
    ...
    ;;
  *)
    echo "Error: Unsupported OS '$OS'. aster currently supports Linux, macOS, and Windows."
    exit 1
    ;;
esac
...
if [ "$OS" = "darwin" ]; then
  FILE="aster-$ARCH-apple-darwin.tar.bz2"
  EXTRACT_CMD="tar"
  ...
  FILE="aster-$ARCH-unknown-linux-gnu.tar.bz2"
  EXTRACT_CMD="tar"
fi

DOWNLOAD_URL="https://github.com/$REPO/releases/download/$RELEASE_TAG/$FILE"

# --- 4) Download & extract 'aster' binary ---
...
TMP_DIR="/tmp/aster_install_$RANDOM"
...
# Windows releases may contain files in a 'aster-package' subdirectory
EXTRACT_DIR="$TMP_DIR"
if [ "$OS" = "windows" ] && [ -d "$TMP_DIR/aster-package" ]; then
  echo "Found aster-package subdirectory, using that as extraction directory"
  EXTRACT_DIR="$TMP_DIR/aster-package"
fi

# Make binary executable
if [ "$OS" = "windows" ]; then
  chmod +x "$EXTRACT_DIR/aster.exe"
else
  chmod +x "$EXTRACT_DIR/aster"
fi

# --- 5) Install to $ASTER_BIN_DIR ---
if [ ! -d "$ASTER_BIN_DIR" ]; then
  echo "Creating directory: $ASTER_BIN_DIR"
  mkdir -p "$ASTER_BIN_DIR"
fi

echo "Moving aster to $ASTER_BIN_DIR/$OUT_FILE"
if [ "$OS" = "windows" ]; then
  mv "$EXTRACT_DIR/aster.exe" "$ASTER_BIN_DIR/$OUT_FILE"
else
  mv "$EXTRACT_DIR/aster" "$ASTER_BIN_DIR/$OUT_FILE"
fi
...
if [ "$CONFIGURE" = true ]; then
  # --- 6) Configure aster (Optional) ---
  echo ""
  echo "Configuring aster"
  echo ""
  "$ASTER_BIN_DIR/$OUT_FILE" configure
else
  echo "Skipping 'aster configure', you may need to run this manually later"
fi



# --- 7) Check PATH and give instructions if needed ---
if [[ ":$PATH:" != *":$ASTER_BIN_DIR:"* ]]; then
  echo ""
  echo "Warning: aster installed, but $ASTER_BIN_DIR is not in your PATH."
  
  if [ "$OS" = "windows" ]; then
    echo "To add aster to your PATH in PowerShell:"
    echo ""
    ...
    if [ "$CONFIGURE" = true ]; then
      ...
      1)
        ...
        echo "Adding \$ASTER_BIN_DIR to $RC_FILE..."
        echo "export PATH=\"$ASTER_BIN_DIR:\$PATH\"" >> "$RC_FILE"
      ...
      2)
        ...
        echo "    export PATH=\"$ASTER_BIN_DIR:\$PATH\""
        echo "Then reload your shell (e.g. 'source ~/.${SHELL_NAME}rc') to apply changes."
        ;;
      *)
        echo "Invalid choice. Please add \$ASTER_BIN_DIR to your PATH manually."
        ;;
      esac
    else
      echo ""
      echo "Configure disabled. Please add \$ASTER_BIN_DIR to your PATH manually."
    fi
