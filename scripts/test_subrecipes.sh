...
# Add aster binary to PATH so subagents can find it when spawning
...
export ASTER_PROVIDER="${ASTER_PROVIDER:-anthropic}"
export ASTER_MODEL="${ASTER_MODEL:-claude-3-5-haiku-20241022}"

echo "Using provider: $ASTER_PROVIDER"
echo "Using model: $ASTER_MODEL"
echo ""
...
TMPFILE=$(mktemp)
if (cd "$TESTDIR" && "$SCRIPT_DIR/target/release/aster" run --recipe project_analyzer_parallel.yaml --no-session 2>&1) | tee "$TMPFILE"; then
