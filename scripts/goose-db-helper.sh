...
BACKUP_DIR="${HOME}/.local/share/aster/aster-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/aster/migrations"
RUST_SESSION_MANAGER="crates/aster/src/session/session_manager.rs"
...
get_aster_db_path() {
    if [[ -n "${ASTER_PATH_ROOT:-}" ]]; then
        echo "${ASTER_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/aster/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/aster/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== Aster Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run Aster yet."
        echo "Once you run Aster, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the aster repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}Aster Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/aster/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
