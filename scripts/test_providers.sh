...
for provider_config in "${PROVIDERS[@]}"; do
  ...
  for MODEL in "${MODELS[@]}"; do
    ...
    echo ""
    TMPFILE=$(mktemp)
    (cd "$TESTDIR" && "$SCRIPT_DIR/target/release/aster" run --text "Immediately use the shell tool to run 'ls'. Do not ask for confirmation." --with-builtin "$BUILTINS" 2>&1) | tee "$TMPFILE"
    echo ""
