#!/usr/bin/env bash
# run-benchmarks.sh - Script to run aster benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting ASTER_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets ASTER_TOOLSHIM_MODEL)"
...
ASTER_CMD="aster"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/aster" ]; then
    ASTER_CMD="./target/debug/aster"
    echo "Using debug binary: $ASTER_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/aster. Falling back to system-installed aster."
  fi
else
  if [ -f "./target/release/aster" ]; then
    ASTER_CMD="./target/release/aster"
    echo "Using release binary: $ASTER_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/aster. Falling back to system-installed aster."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export ASTER_PROVIDER="$provider"
  export ASTER_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export ASTER_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export ASTER_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $ASTER_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
