...
SCRIPT_DIR=$(pwd)
ASTER_BIN="$SCRIPT_DIR/target/release/aster"
...
validate_compaction() {
  ...
  local session_json=$($ASTER_BIN session export --format json --session-id "$session_id" 2>&1)
...
(cd "$TESTDIR" && "$ASTER_BIN" run --text "list files and read hello.txt" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  SESSION_ID=$("$ASTER_BIN" session list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$ASTER_BIN" run --resume --session-id "$SESSION_ID" --text "Please compact this conversation" 2>&1) | tee -a "$OUTPUT"
...
export ASTER_AUTO_COMPACT_THRESHOLD=0.005

OUTPUT=$(mktemp)

echo "Step 1: Creating session with first message..."
(cd "$TESTDIR" && "$ASTER_BIN" run --text "hello" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$ASTER_BIN" run --resume --session-id "$SESSION_ID" --text "hi again" 2>&1) | tee -a "$OUTPUT"
...
unset ASTER_AUTO_COMPACT_THRESHOLD
...
if ! (cd "$PROXY_DIR" && uv sync 2>&1 | tee "$PROXY_SETUP_LOG"); then
  ...
  if [ "$PROXY_READY" != "true" ]; then
    ...
    export ASTER_PROVIDER_SKIP_BACKOFF=true
    export ASTER_PROVIDER=anthropic
    export ASTER_MODEL=claude-haiku-4-5

    echo "Step 1: Creating session (should trigger context-length error and compaction)..."
    (cd "$TESTDIR" && "$ASTER_BIN" run --text "hello world" 2>&1) | tee "$OUTPUT"
    ...
    unset ASTER_PROVIDER_SKIP_BACKOFF
    unset ASTER_PROVIDER
    unset ASTER_MODEL
    unset UV_INDEX_URL
  fi
fi
