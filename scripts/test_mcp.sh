...
JUDGE_PROVIDER=${ASTER_JUDGE_PROVIDER:-openrouter}
JUDGE_MODEL=${ASTER_JUDGE_MODEL:-google/gemini-2.5-flash}
...
for provider_config in "${PROVIDERS[@]}"; do
  ...
  for i in $(seq 1 $((${#PARTS[@]} - 1))); do
    ...
    echo ""
    TMPFILE=$(mktemp)
    (cd "$TESTDIR" && "$SCRIPT_DIR/target/release/aster" run --text "Use the sampleLLM tool to ask for a quote from The Great Gatsby" --with-extension "npx -y @modelcontextprotocol/server-everything" 2>&1) | tee "$TMPFILE"
    echo ""
...
EOF
      ...
      JUDGE_OUT=$(ASTER_PROVIDER="$JUDGE_PROVIDER" ASTER_MODEL="$JUDGE_MODEL" \
        "$SCRIPT_DIR/target/release/aster" run --text "$JUDGE_PROMPT" 2>&1)
